...
theorem isOrderRightAdjoint_sSup [PartialOrder α] [CompleteLattice α] [Preorder β] (f : α → β) :
    IsOrderRightAdjoint f fun y => sSup { x | f x ≤ y } := fun _ => isLUB_sSup _

theorem isOrderRightAdjoint_csSup [PartialOrder α] [CompleteLattice α] [Preorder β] (f : α → β)
...
cohomologie bornée][ghys87:groupes]. -/
theorem sSup_div_semiconj [PartialOrder α] [CompleteLattice α] [Group G]
    (f₁ f₂ : G →* α ≃o α) (g : G) :
