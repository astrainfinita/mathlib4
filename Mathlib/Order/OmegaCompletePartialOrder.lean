...
omit [OmegaCompletePartialOrder α] [OmegaCompletePartialOrder β] [OmegaCompletePartialOrder γ]
...
instance (priority := 100) [PartialOrder α] [CompleteLattice α] : OmegaCompletePartialOrder α where
  ωSup c := ⨆ i, c i
  ωSup_le := fun ⟨c, _⟩ s hs => by simpa only [iSup_le_iff]
  le_ωSup := fun ⟨c, _⟩ i => le_iSup_of_le i le_rfl

variable [SemilatticeSup α] [OmegaCompletePartialOrder α] [Lattice β] [CompleteLattice β]
  {f g : α → β}
...
lemma ωScottContinuous.top [OrderTop β] : ωScottContinuous (⊤ : α → β) :=
  ωScottContinuous.of_monotone_map_ωSup
    ⟨monotone_const, fun c ↦ eq_of_forall_ge_iff fun a ↦ by simp⟩

lemma ωScottContinuous.bot [OrderBot β] : ωScottContinuous (⊥ : α → β) := by
...
variable [PartialOrder α] [OmegaCompletePartialOrder α] [LinearOrder β] [CompleteLattice β]
  {f g : α → β}
