...
boundedness. -/
class ConditionallyCompleteLattice (α : Type*) [PartialOrder α] extends
    OrderSupSet α, OrderInfSet α where
...
hold in both worlds, sometimes with additional assumptions of nonemptiness or
boundedness. -/
@[deprecated "Use `[LinearOrder α] [ConditionallyCompleteLinearOrder α] [OrderBot α]` instead"
  (since := "2026-04-24")]
structure ConditionallyCompleteLinearOrderBot (α : Type*) extends LinearOrder α,
    ConditionallyCompleteLinearOrder α, OrderBot α where
...
def conditionallyCompleteLatticeOfLatticeOfsSup (α : Type*) [H1 : PartialOrder α] [OrderSupSet α]
    (isLUB_sSup : ∀ s : Set α, BddAbove s → s.Nonempty → IsLUB s (sSup s)) :
