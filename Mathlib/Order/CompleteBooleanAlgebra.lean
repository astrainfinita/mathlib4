...
noncomputable abbrev ofMinimalAxioms (minAx : MinimalAxioms α) : Frame α where
  __ := minAx
  compl a := sSup {c | c ⊓ a ≤ ⊥}
...
-- See note [reducible non-instances]
noncomputable abbrev ofMinimalAxioms (minAx : MinimalAxioms α) : Coframe α where
  __ := minAx
  hnot a := sInf {c | ⊤ ≤ a ⊔ c}
...
`￢a := ⊤ \ a`. -/
-- See note [reducible non-instances]
noncomputable abbrev ofMinimalAxioms (minAx : MinimalAxioms α) : CompleteDistribLattice α where
...
`￢a := ⊤ \ a`. -/
-- See note [reducible non-instances]
noncomputable abbrev ofMinimalAxioms (minAx : MinimalAxioms α) : CompletelyDistribLattice α where
  __ := minAx
...
protected abbrev Function.Injective.frameMinimalAxioms [Max α] [Min α] (sSup' sInf' : Set α → α)
    [Top α] [Bot α] (minAx : Frame.MinimalAxioms β) (f : α → β) (hf : Injective f)
    (map_sup : ∀ a b, f (a ⊔ b) = f a ⊔ f b) (map_inf : ∀ a b, f (a ⊓ b) = f a ⊓ f b)
    (map_sSup' : ∀ s, f (sSup' s) = ⨆ a ∈ s, f a) (map_sInf' : ∀ s, f (sInf' s) = ⨅ a ∈ s, f a)
    (map_top : f ⊤ = ⊤) (map_bot : f ⊥ = ⊥) : Frame.MinimalAxioms α where
  __ := hf.completeLattice sSup' sInf' f map_sup map_inf map_sSup' map_sInf' map_top map_bot
  inf_sSup_le_iSup_inf a s := by
    change f (a ⊓ sSup s) ≤ f _
    have isLUB_sSup' (s) : IsLUB s (sSup' s) :=
      ⟨fun a h ↦ (le_iSup₂ a h).trans (map_sSup' _).ge,
        fun _ h ↦ (map_sSup' _).trans_le <| iSup₂_le h⟩
    rw [← sSup_image, map_inf, (isLUB_sSup' _).sSup_eq, (isLUB_sSup' _).sSup_eq,
      map_sSup' s, minAx.inf_iSup₂_eq]
    simp_rw [← map_inf]
    exact ((map_sSup' _).trans iSup_image).ge

-- See note [reducible non-instances]
/-- Pullback an `Order.Coframe.MinimalAxioms` along an injection. -/
protected abbrev Function.Injective.coframeMinimalAxioms [Max α] [Min α] (sSup' sInf' : Set α → α)
  ...
  iInf_sup_le_sup_sInf a s := by
    ...
    have isGLB_sInf' (s) : IsGLB s (sInf' s) :=
      ⟨fun a h ↦ (map_sInf' _).trans_le <| iInf₂_le a h,
        fun _ h ↦ (le_iInf₂ (f := fun i _ ↦ f i) h).trans ((map_sInf' s).ge)⟩
    rw [← sInf_image, map_sup, (isGLB_sInf' _).sInf_eq, (isGLB_sInf' _).sInf_eq,
      map_sInf' s, minAx.sup_iInf₂_eq]
    simp_rw [← map_sup]
    exact ((map_sInf' _).trans iInf_image).le

-- See note [reducible non-instances]
/-- Pullback an `Order.Frame` along an injection. -/
protected abbrev Function.Injective.frame [Max α] [Min α] (sSup' sInf' : Set α → α) [Top α] [Bot α]
    [Compl α] [HImp α] [Frame β] (f : α → β) (hf : Injective f)
    (map_sup : ∀ a b, f (a ⊔ b) = f a ⊔ f b) (map_inf : ∀ a b, f (a ⊓ b) = f a ⊓ f b)
    (map_sSup' : ∀ s, f (sSup' s) = ⨆ a ∈ s, f a) (map_sInf' : ∀ s, f (sInf' s) = ⨅ a ∈ s, f a)
    (map_top : f ⊤ = ⊤) (map_bot : f ⊥ = ⊥) (map_compl : ∀ a, f aᶜ = (f a)ᶜ)
    (map_himp : ∀ a b, f (a ⇨ b) = f a ⇨ f b) : Frame α where
  __ := hf.frameMinimalAxioms sSup' sInf' .of f map_sup map_inf map_sSup' map_sInf' map_top map_bot
  __ := hf.heytingAlgebra f map_sup map_inf map_top map_bot map_compl map_himp

-- See note [reducible non-instances]
/-- Pullback an `Order.Coframe` along an injection. -/
protected abbrev Function.Injective.coframe [Max α] [Min α] (sSup' sInf' : Set α → α)
  ...
  __ := hf.coframeMinimalAxioms sSup' sInf' .of f map_sup map_inf map_sSup' map_sInf map_top map_bot
...
protected abbrev Function.Injective.completeDistribLatticeMinimalAxioms [Max α] [Min α]
    (sSup' sInf' : Set α → α)
    [Top α] [Bot α] (minAx : CompleteDistribLattice.MinimalAxioms β) (f : α → β)
    (hf : Injective f)
  ...
  __ := hf.frameMinimalAxioms sSup' sInf' minAx.toFrame f
    map_sup map_inf map_sSup' map_sInf' map_top map_bot
  __ := hf.coframeMinimalAxioms sSup' sInf' minAx.toCoframe f
    map_sup map_inf map_sSup' map_sInf' map_top map_bot

-- See note [reducible non-instances]
/-- Pullback a `CompleteDistribLattice` along an injection. -/
protected abbrev Function.Injective.completeDistribLattice [Max α] [Min α] (sSup' sInf' : Set α → α)
  ...
  __ := hf.frame sSup' sInf' f
    map_sup map_inf map_sSup' map_sInf' map_top map_bot map_compl map_himp
  __ := hf.coframe sSup' sInf' f
    map_sup map_inf map_sSup' map_sInf' map_top map_bot map_hnot map_sdiff
...
protected abbrev Function.Injective.completelyDistribLatticeMinimalAxioms [Max α] [Min α]
    (sSup' sInf' : Set α → α) [Top α] [Bot α] (minAx : CompletelyDistribLattice.MinimalAxioms β)
  ...
  __ := (hf.completeDistribLatticeMinimalAxioms sSup' sInf' minAx.toCompleteDistribLattice f
    map_sup map_inf map_sSup' map_sInf' map_top map_bot).toCompleteLattice
  iInf_iSup_eq g := hf <| by
    ...
    have isGLB_sInf' (s) : IsGLB s (sInf' s) :=
      ⟨fun a h ↦ (map_sInf' _).trans_le <| iInf₂_le a h,
        fun _ h ↦ (le_iInf₂ (f := fun i _ ↦ f i) h).trans ((map_sInf' s).ge)⟩
    simp_rw [iInf, (isGLB_sInf' _).sInf_eq, map_sInf', iInf_range, iSup, (isLUB_sSup' _).sSup_eq,
      map_sSup', iSup_range, map_sInf', iInf_range, minAx.iInf_iSup_eq']
  ...
  iInf_iSup_eq g := hf <| by
    ...
    have isGLB_sInf' (s) : IsGLB s (sInf' s) :=
      ⟨fun a h ↦ (map_sInf' _).trans_le <| iInf₂_le a h,
        fun _ h ↦ (le_iInf₂ (f := fun i _ ↦ f i) h).trans ((map_sInf' s).ge)⟩
    simp_rw [iInf, (isGLB_sInf' _).sInf_eq, map_sInf', iInf_range, iSup, (isLUB_sSup' _).sSup_eq,
      map_sSup', iSup_range, map_sInf', iInf_range, iInf_iSup_eq]

-- See note [reducible non-instances]
/-- Pullback a `CompleteBooleanAlgebra` along an injection. -/
protected abbrev Function.Injective.completeBooleanAlgebra [Max α] [Min α] (sSup' sInf' : Set α → α)
...
protected abbrev Function.Injective.completeAtomicBooleanAlgebra [Max α] [Min α]
    (sSup' sInf' : Set α → α) [Top α] [Bot α] [Compl α] [HImp α] [HNot α] [SDiff α]
  ...
  __ := hf.completelyDistribLattice sSup' sInf' f
    map_sup map_inf map_sSup' map_sInf' map_top map_bot map_compl map_himp map_hnot map_sdiff
  __ := hf.booleanAlgebra f map_sup map_inf map_top map_bot map_compl map_sdiff map_himp
