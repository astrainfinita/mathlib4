...
@[to_dual]
instance (priority := 50) CompleteSemilatticeSup.toNonempty : Nonempty α :=
  (CompleteSemilatticeSup.exists_isLUB ∅).nonempty

@[to_dual]
theorem isLUB_sSup (s : Set α) : IsLUB s (sSup s) :=
  isLUB_sSup_of_exists (CompleteSemilatticeSup.exists_isLUB s)
...
instance {α : Type*} [CompleteSemilatticeInf α] : CompleteSemilatticeSup αᵒᵈ where
  exists_isLUB := CompleteSemilatticeInf.exists_isGLB (α := α)
...
attribute [to_dual existing] CompleteLattice.exists_isLUB
attribute [to_dual self (reorder := exists_isLUB exists_isGLB)] CompleteLattice.mk
...
/-- Create a `CompleteLattice` from a `PartialOrder` and `sInf`
...
def completeLatticeOfInf (α : Type*) [H1 : PartialOrder α] (sInf : Set α → α)
  ...
  exists_isLUB s := ⟨_, isGLB_upperBounds.mp (isGLB_sInf _)⟩

/-- Create a `CompleteLattice` from a `PartialOrder` and `sSup`
...
def completeLatticeOfSup (α : Type*) [H1 : PartialOrder α] (sSup : Set α → α)
  ...
  exists_isGLB s := ⟨_, isLUB_lowerBounds.mp (isLUB_sSup _)⟩

attribute [to_dual existing] completeLatticeOfSup
...
noncomputable def completeLatticeOfCompleteSemilatticeInf (α : Type*) [CompleteSemilatticeInf α] :
    CompleteLattice α :=
  completeLatticeOfInf α sInf isGLB_sInf
