...
an addition. -/]
def sInf' (S : Set (Con M)) : Con M where
...
@[to_additive]
lemma isGLB_sInf' (S : Set (Con M)) : IsGLB S (sInf' S) :=
...
theorem coe_sInf (S : Set (Con M)) :
    ⇑(sInf S) = sInf ((⇑) '' S) := by
  ext
  simp only [(isGLB_sInf' _).sInf_eq, sInf_image, iInf_apply, iInf_Prop_eq]
  rfl
...
@[to_additive]
theorem sInf_iff {s : Set (Con M)} {x y : M} :
...
instance : CompleteLattice (Con M) where
  __ := completeLatticeOfInf (Con M) _ isGLB_sInf'
...
theorem sup_eq_conGen (c d : Con M) : c ⊔ d = conGen fun x y => c x y ∨ d x y := by
  rw [conGen_eq, (isGLB_sInf' _).sInf_eq]
...
theorem sSup_eq_conGen (S : Set (Con M)) :
  ...
  rw [conGen_eq, ← sInf_upperBounds_eq_sSup]
  apply congr_arg sInf
  ext
