...
noncomputable def closure (R : Type*) {M : Type*} [SMul R M] (s : Set M) : SubMulAction R M :=
  sInf { p | s ⊆ (p : Set M) }

variable {R M : Type*} [SMul R M] {s t : Set M} {x : M}

@[to_additive]
theorem mem_closure : x ∈ closure R s ↔ ∀ p : SubMulAction R M, s ⊆ (p : Set M) → x ∈ p := by
  simp [closure, ← SetLike.mem_coe, (SubMulAction.preservesGLB _).map_sInf]
