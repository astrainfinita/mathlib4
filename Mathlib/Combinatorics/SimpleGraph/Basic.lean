...
theorem isLUB_mk_exists (s : Set (SimpleGraph V)) :
    IsLUB s
...
theorem isGLB_mk_forall (s : Set (SimpleGraph V)) :
    IsGLB s
...
instance completeAtomicBooleanAlgebra : CompleteAtomicBooleanAlgebra (SimpleGraph V) where
  ...
  top_le_sup_compl G v w hvw := by
    ...
    · exact Or.inl h
    · exact Or.inr ⟨hvw, h⟩
  exists_isLUB _ := ⟨_, isLUB_mk_exists _⟩
  exists_isGLB _ := ⟨_, isGLB_mk_forall _⟩
