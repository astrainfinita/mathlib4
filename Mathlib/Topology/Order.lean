...
instance : Lattice (TopologicalSpace α) := (gciGenerateFrom α).liftLattice

/-- Topologies on `α` form a `BoundedOrder`, with `⊥` the discrete topology
  and `⊤` the indiscrete topology. -/
instance : BoundedOrder (TopologicalSpace α) := (gciGenerateFrom α).liftBoundedOrder

/-- Topologies on `α` form a complete lattice. The infimum of a collection of topologies
