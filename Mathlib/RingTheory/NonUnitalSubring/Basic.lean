...
noncomputable def sInf' (s : Set (NonUnitalSubring R)) : NonUnitalSubring R :=
  NonUnitalSubring.mk' (⋂ t ∈ s, ↑t) (⨅ t ∈ s, NonUnitalSubring.toSubsemigroup t)
    (⨅ t ∈ s, NonUnitalSubring.toAddSubgroup t) (by simp) (by simp)

lemma isGLB_sInf' (s : Set (NonUnitalSubring R)) : IsGLB s (sInf' s) :=
...
theorem sInf_toAddSubgroup (s : Set (NonUnitalSubring R)) :
  ...
  (isGLB_sInf' s).sInf_eq ▸ mk'_toAddSubgroup _ _

/-- `NonUnitalSubring`s of a ring form a complete lattice. -/
noncomputable instance : CompleteLattice (NonUnitalSubring R) :=
  { completeLatticeOfInf (NonUnitalSubring R) _ isGLB_sInf' with
    bot := ⊥
...
noncomputable def closure (s : Set R) : NonUnitalSubring R :=
  sInf {S | s ⊆ S}
...
noncomputable abbrev closureNonUnitalCommRingOfComm {R : Type*} [NonUnitalRing R] {s : Set R}
...
protected noncomputable def gi : GaloisInsertion (@closure R _) SetLike.coe where
