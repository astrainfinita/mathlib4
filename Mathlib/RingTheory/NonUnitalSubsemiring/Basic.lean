...
noncomputable def sInf' (s : Set (NonUnitalSubsemiring R)) : NonUnitalSubsemiring R :=
...
lemma isGLB_sInf' (s : Set (NonUnitalSubsemiring R)) : IsGLB s (sInf' s) :=
...
theorem mem_sInf {S : Set (NonUnitalSubsemiring R)} {x : R} : x ∈ sInf S ↔ ∀ p ∈ S, x ∈ p :=
  (isGLB_sInf' S).sInf_eq ▸ Set.mem_iInter₂
...
theorem sInf_toSubsemigroup (s : Set (NonUnitalSubsemiring R)) :
  ...
  (isGLB_sInf' s).sInf_eq ▸ mk'_toSubsemigroup _ _

@[simp]
theorem sInf_toAddSubmonoid (s : Set (NonUnitalSubsemiring R)) :
    (sInf s).toAddSubmonoid = ⨅ t ∈ s, NonUnitalSubsemiring.toAddSubmonoid t :=
  (isGLB_sInf' s).sInf_eq ▸ mk'_toAddSubmonoid _ _

/-- Non-unital subsemirings of a non-unital semiring form a complete lattice. -/
noncomputable instance : CompleteLattice (NonUnitalSubsemiring R) := fast_instance%
  { completeLatticeOfInf (NonUnitalSubsemiring R) _ isGLB_sInf' with
    bot := ⊥
...
noncomputable def closure (s : Set R) : NonUnitalSubsemiring R :=
  sInf { S | s ⊆ S }
...
noncomputable abbrev closureNonUnitalCommSemiringOfComm {R : Type*}
    [NonUnitalSemiring R] {s : Set R}
...
noncomputable def nonUnitalSubsemiringClosure (M : Subsemigroup R) : NonUnitalSubsemiring R :=
...
theorem nonUnitalSubsemiringClosure_eq_closure :
  ...
  all_goals
    simp only [nonUnitalSubsemiringClosure_coe, SetLike.mem_coe,
      NonUnitalSubsemiring.mem_closure, AddSubmonoid.mem_closure]
    intro H1 H2
...
protected noncomputable def gi : GaloisInsertion (@closure R _) (↑) where
