...
@[to_additive]
lemma preservesGLB (s : Set (Subgroup G)) : PreservesGLB ((↑) : _ → Set G) s := by
...
theorem coe_sInf (H : Set (Subgroup G)) : ((sInf H : Subgroup G) : Set G) = ⋂ s ∈ H, ↑s :=
  (preservesGLB H).map_sInf_eq_biInf

@[to_additive (attr := simp)]
theorem mem_sInf {S : Set (Subgroup G)} {x : G} : x ∈ sInf S ↔ ∀ p ∈ S, x ∈ p := by
...
noncomputable instance : CompleteLattice (Subgroup G) :=
  { completeLatticeOfInf (Subgroup G) _ fun s ↦ (preservesGLB s).isGLB_sInf with
    bot := ⊥
...
noncomputable def closure (k : Set G) : Subgroup G :=
  sInf { K | k ⊆ K }
...
protected noncomputable def gi : GaloisInsertion (@closure G _) (↑) where
