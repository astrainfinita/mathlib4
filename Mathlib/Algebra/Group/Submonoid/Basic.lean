...
instance : CompleteLattice (Submonoid M) :=
  (completeLatticeOfInf (Submonoid M)) fun _ => .of_image SetLike.coe_subset_coe isGLB_biInf

@[to_additive]
instance : Lattice (Submonoid M) where
  inf := (· ⊓ ·)
...
@[to_additive]
instance : BoundedOrder (Submonoid M) where
