...
@[to_additive]
lemma preservesGLB (s : Set (Submonoid M)) : PreservesGLB ((↑) : _ → Set M) s := by
...
theorem mem_sInf {S : Set (Submonoid M)} {x : M} : x ∈ sInf S ↔ ∀ p ∈ S, x ∈ p := by
...
noncomputable instance : CompleteLattice (Submonoid M) :=
  { (completeLatticeOfInf (Submonoid M)) _ fun s ↦ (preservesGLB s).isGLB_sInf with
    le := (· ≤ ·)
    lt := (· < ·)
    bot := ⊥
...
noncomputable def closure (s : Set M) : Submonoid M :=
  sInf { S | s ⊆ S }
...
protected noncomputable def gi : GaloisInsertion (@closure M _) SetLike.coe where
