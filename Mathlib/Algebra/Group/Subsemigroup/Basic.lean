...
@[to_additive]
def sInf' (s : Set (Subsemigroup M)) : Subsemigroup M where
  carrier := ⋂ t ∈ s, ↑t
  mul_mem' := fun hx hy =>
...
@[to_additive]
def isGLB_sInf' (s : Set (Subsemigroup M)) : IsGLB s (sInf' s) :=
...
instance : CompleteLattice (Subsemigroup M) :=
  { completeLatticeOfInf (Subsemigroup M) _ isGLB_sInf' with
    le := (· ≤ ·)
    lt := (· < ·)
    bot := ⊥
...
noncomputable def closure (s : Set M) : Subsemigroup M :=
  sInf { S | s ⊆ S }
...
protected noncomputable def gi : GaloisInsertion (@closure M _) SetLike.coe :=
