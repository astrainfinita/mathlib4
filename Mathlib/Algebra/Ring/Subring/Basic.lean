...
lemma preservesGLB (s : Set (Subring R)) : PreservesGLB ((↑) : _ → Set R) s := by
  refine .of_isGLB_image SetLike.coe_subset_coe ?_ (isGLB_sInf _)
  use Subring.mk' (⋂ t ∈ s, ↑t) (⨅ t ∈ s, t.toSubmonoid) (⨅ t ∈ s, Subring.toAddSubgroup t)
    (by simp) (by simp)
  simp

@[simp, norm_cast]
theorem coe_sInf (S : Set (Subring R)) : ((sInf S : Subring R) : Set R) = ⋂ s ∈ S, ↑s :=
  (preservesGLB S).map_sInf_eq_biInf

@[simp]
theorem mem_sInf {S : Set (Subring R)} {x : R} : x ∈ sInf S ↔ ∀ p ∈ S, x ∈ p := by
...
noncomputable instance : CompleteLattice (Subring R) :=
  { completeLatticeOfInf (Subring R) _ fun s ↦ (preservesGLB s).isGLB_sInf with
    bot := ⊥
    bot_le := fun s _x hx =>
...
noncomputable def closure (s : Set R) : Subring R :=
  sInf { S | s ⊆ S }
...
noncomputable abbrev closureCommRingOfComm {R} [Ring R] {s : Set R}
    (hcomm : ∀ x ∈ s, ∀ y ∈ s, x * y = y * x) :
    CommRing (closure s) :=
