...
noncomputable def sInf' (s : Set (Subsemiring R)) : Subsemiring R :=
  Subsemiring.mk' (⋂ t ∈ s, ↑t) (⨅ t ∈ s, Subsemiring.toSubmonoid t) (by simp)
    (⨅ t ∈ s, Subsemiring.toAddSubmonoid t) (by simp)

lemma isGLB_sInf' (s : Set (Subsemiring R)) : IsGLB s (sInf' s) :=
...
theorem sInf_toSubmonoid (s : Set (Subsemiring R)) :
  ...
  (isGLB_sInf' s).sInf_eq ▸ mk'_toSubmonoid _ _
...
noncomputable instance : CompleteLattice (Subsemiring R) :=
  { completeLatticeOfInf (Subsemiring R) _ isGLB_sInf' with
    bot := ⊥
    bot_le := fun s _ hx =>
...
noncomputable def closure (s : Set R) : Subsemiring R :=
  sInf { S | s ⊆ S }
...
noncomputable def subsemiringClosure (M : Submonoid R) : Subsemiring R :=
...
theorem subsemiringClosure_eq_closure : M.subsemiringClosure = Subsemiring.closure (M : Set R) := by
  ...
  all_goals
    simp only [SetLike.mem_coe, Submonoid.subsemiringClosure_coe, Subsemiring.mem_closure,
...
noncomputable abbrev closureCommSemiringOfComm {s : Set R'}
