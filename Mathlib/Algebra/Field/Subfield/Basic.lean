...
lemma preservesGLB (S : Set (Subfield K)) : PreservesGLB ((↑) : _ → Set K) S := by
...
noncomputable instance : CompleteLattice (Subfield K) :=
  { completeLatticeOfInf (Subfield K) _ fun s ↦ (preservesGLB s).isGLB_sInf with
    top := ⊤
...
noncomputable def closure (s : Set K) : Subfield K := sInf {S | s ⊆ S}
...
protected noncomputable def gi : GaloisInsertion (@closure K _) (↑) where
