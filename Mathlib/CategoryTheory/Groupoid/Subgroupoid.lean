...
def sInf' (s : Set (Subgroupoid C)) : Subgroupoid C where
...
theorem mem_sInf'_arrows {s : Set (Subgroupoid C)} {c d : C} {p : c ⟶ d} :
...
theorem isGLB_sInf' (s : Set (Subgroupoid C)) : IsGLB s (sInf' s) := by
  refine ⟨fun S Ss F => ?_, fun T Tl F fT => ?_⟩ <;> simp only [mem_sInf']
  exacts [fun hp => hp S Ss, fun S Ss => Tl Ss fT]

theorem mem_sInf_arrows {s : Set (Subgroupoid C)} {c d : C} {p : c ⟶ d} :
    p ∈ (sInf s).arrows c d ↔ ∀ S ∈ s, p ∈ S.arrows c d :=
  (isGLB_sInf' s).sInf_eq ▸ mem_iInter₂
...
instance : CompleteLattice (Subgroupoid C) :=
  { completeLatticeOfInf (Subgroupoid C) _ isGLB_sInf' with
    bot := ⊥
...
theorem sInf_isNormal (s : Set <| Subgroupoid C) (sn : ∀ S ∈ s, IsNormal S) : IsNormal (sInf s) :=
  { wide := by simp_rw [mem_sInf_arrows]; exact fun c S Ss => (sn S Ss).wide c
    conj := by simp_rw [mem_sInf_arrows]; exact fun p γ hγ S Ss => (sn S Ss).conj p (hγ S Ss) }
...
noncomputable def generated : Subgroupoid C :=
...
theorem subset_generated (c d : C) : X c d ⊆ (generated X).arrows c d := by
  ...
  simp only [(isGLB_sInf' _).sInf_eq, sInf', subset_iInter₂_iff]
  exact fun S hS f fS => hS _ _ fS

/-- The normal subgroupoid generated by the set of arrows `X` -/
noncomputable def generatedNormal : Subgroupoid C :=
