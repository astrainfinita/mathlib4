...
def tree (A : Type*) : CompleteSublattice (Set (List A)) :=
  CompleteSublattice.mk' {T | ∀ ⦃x : List A⦄ ⦃a : A⦄, x ++ [a] ∈ T → x ∈ T}
    (by
      simp only [Set.sSup_eq_sUnion, Set.mem_setOf_eq, Set.mem_sUnion]
      rintro S hS x a ⟨t, ht, hx⟩; use t, ht, hS ht hx)
    (by
      simp only [Set.sInf_eq_sInter, Set.mem_setOf_eq, Set.mem_sInter]
      rintro S hS x a h T hT; exact hS hT <| h T hT)
